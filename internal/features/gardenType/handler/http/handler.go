...
type Handler struct {
	UseCase gardenTypeDomain.GardenTypeUseCase
}

func NewHandler(e *echo.Echo, useCase gardenTypeDomain.GardenTypeUseCase) {
	handler := &Handler{
		UseCase: useCase,
	}
...
func (m *Handler) Create(e echo.Context) error {
	var gardenType gardenTypeDomain.GardenType
...
func (m *Handler) Update(e echo.Context) error {
	var gardenType gardenTypeDomain.GardenTypeForm
	if err := e.Bind(&gardenType); err != nil {
		return e.JSON(http.StatusBadRequest, err.Error())
	}
